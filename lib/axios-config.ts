...
// ============================================================================
// Google Vertex AI (Imagen) 专用的 axios 实例
// ============================================================================
...
console.log('[Google Vertex AI Config] Base URL:', VERTEX_AI_BASE_URL || 'Not configured');
console.log('[Google Vertex AI Config] Project ID:', GOOGLE_PROJECT_ID || 'Not set');
console.log('[Google Vertex AI Config] Location:', GOOGLE_LOCATION);

const googleAxios: AxiosInstance = axios.create({
...
// Google Vertex AI 请求拦截器
googleAxios.interceptors.request.use(
  async (config: InternalAxiosRequestConfig) => {
    const fullUrl = `${config.baseURL}${config.url}`;

    // 使用 Google Cloud ADC (Application Default Credentials) 获取访问令牌
    try {
      // 动态导入 google-auth-library (减小客户端包大小)
      ...
      log(`\n[Google Vertex AI Request] ${config.method?.toUpperCase()} ${fullUrl}`);
      log('Using OAuth2 access token');
    } catch (error: any) {
      logError('[Google Vertex AI] Failed to get access token:', error.message);
      logError('[Google Vertex AI] Please ensure:');
      logError('  1. GOOGLE_APPLICATION_CREDENTIALS environment variable is set, OR');
      logError('  2. Application is running in Google Cloud environment with Workload Identity, OR');
      logError('  3. gcloud auth application-default login has been run (for local development)');
  ...
  (error: AxiosError) => {
    logError('[Google Vertex AI Request Error]', error.message);
    return Promise.reject(error);
  }
);

// Google Vertex AI 响应拦截器
googleAxios.interceptors.response.use(
  (response) => {
    log(`[Google Vertex AI Response] ${response.status} ${response.statusText}`);
    if (response.data) {
      // Imagen 返回 base64 编码的图片，不打印完整数据
      const predictions = (response.data as any).predictions;
      if (predictions && predictions.length > 0) {
        log(`Response data: ${predictions.length} prediction(s) received with base64-encoded images`);
      } else {
        log('Response data:', response.data);
      }
    }
  ...
  (error: AxiosError) => {
    if (error.response) {
      logError(`[Google Vertex AI Response Error] ${error.response.status} ${error.response.statusText}`);
      logError('Error data:', error.response.data);
    } else if (error.request) {
      logError('[Google Vertex AI Network Error] No response received');
      logError('Request details:', {
        url: error.config?.url,
        baseURL: error.config?.baseURL,
        method: error.config?.method,
      ...
      logError('[Google Vertex AI Error]', error.message);
    }
    return Promise.reject(error);
  }
);

export { axiosInstance, aiHubAxios, evolinkAxios, googleAxios };
