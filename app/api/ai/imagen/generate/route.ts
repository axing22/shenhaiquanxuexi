  ...
  try {
    ...
    if (negativePrompt) {
      ...
      if (containsChinese(negativePrompt)) {
        log('[Imagen Generate] 检测到中文负面提示词，开始翻译:', negativePrompt);
        try {
          finalNegativePrompt = await translateText(negativePrompt, 'en');
          log('[Imagen Generate] 负面提示词翻译完成:', finalNegativePrompt);
        } catch (error) {
